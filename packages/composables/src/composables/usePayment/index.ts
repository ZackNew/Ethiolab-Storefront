...
const usePaymentFactoryParams: UsePaymentFactoryParams<PaymentMethodQuote[]> = {
  ...
  set: async (context: Context, { method, metadata, customQuery }) => {
    const response = await context.$vendure.api.setPaymentMethod(
      { method, metadata },
      customQuery
    );
  ...
  load: async (
    ...
    const response = await context.$vendure.api.getPaymentMethods(
      params?.customQuery
    );
...
const usePayment: () => UsePayment<PaymentMethodQuote[]> = usePaymentFactory<
  PaymentMethodQuote[]
>(usePaymentFactoryParams);
