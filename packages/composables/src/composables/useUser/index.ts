...
const params: UseUserFactoryParams<any, UpdateParams, RegisterParams> = {
  ...
  updateUser: async (context: Context, { currentUser, updatedUserData }) => {
    const response = await context.$vendure.api.updateCustomer({
  ...
  register: async (
    ...
    const response = await context.$vendure.api.registerCustomerAccount({
      emailAddress: email,
      password,
      firstName,
      lastName,
  ...
  changePassword: async (
    ...
    const response = await context.$vendure.api.updateCustomerPassword({
      currentPassword,
      newPassword,
    });
  ...
  updateEmail: async (
    context: Context,
    { currentUser, updatedUserData, newEmail, password }
  ) => {
    const response = await context.$vendure.api.updateCustomerEmailAddress({
      newEmail,
      password,
    });
...
export const useUser = useUserFactory<any, UpdateParams, RegisterParams>(
  params
);
