...
interface ExtendedSearchGetters
  extends FacetsGetters<AgnosticSearchResult, FacetSearchCriteria> {
  getTree: (category: Collection) => AgnosticCategoryTree | null;
  getBreadcrumbsFromSlug: (
    searchResult: FacetSearchResult<AgnosticSearchResult>,
    slug: string
  ) => AgnosticBreadcrumb[];
  getAgnosticSearchResult: (
    searchResultValue: SearchResultValue<SearchResponse, SearchInput>
  ) => FacetSearchResult<AgnosticSearchResult>;
}
...
const getProducts = (
  ...
  return (
    ...
    searchResult.data.products.map((product) => ({
...
const getPagination = (
  ...
  return {
    totalPages:
      Math.ceil(searchResult?.data?.total / searchResult?.data?.itemsPerPage) ||
      1,
...
const getBreadcrumbsFromSlug = (
  searchResult: FacetSearchResult<AgnosticSearchResult>,
  slug: string
): AgnosticBreadcrumb[] => {
  const categoryFromSlug = searchResult?.data?.categories?.find(
    (category) => category.collection.slug === slug
  );

  const breadcrumbsFromSlug = categoryFromSlug?.collection?.breadcrumbs?.map(
    (breadcrumb) => ({
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const getAll = (
  params: FacetSearchResult<AgnosticSearchResult>,
