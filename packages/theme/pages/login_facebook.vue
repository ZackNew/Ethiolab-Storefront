  ...
  mounted() {
    ...
    const login = async () => {
      const addtionalData = await axios.get(`
                  https://graph.facebook.com/v14.0/oauth/access_token?redirect_uri=${encodeURIComponent(
                    'http://localhost:3001/login_facebook'
                  )}&client_id=980997845873215&client_secret=9bab1bb398d27f2a67ff1708ba83801f&code=${
      ...
      const mutation = gql`
        mutation Authenticate($accessToken: String!) {
          authenticate(
            input: { facebook: { token: $accessToken } }
            rememberMe: true
          ) {
            ... on CurrentUser {
              id
