<template>
  ...
  <SfAccordion class="accordion" :open=openState :multiple="false" transition="" showChevron>
    <SfAccordionItem  :header="category.label" >
      <SfList class="ml-5" >
        <SfListItem>
          ...
          >
            <template #label>
              <nuxt-link
                  :to="'/c/'+category.slug"
                  :class="category.isCurrent ? 'sidebar--cat-selected' : 'mb-3 '"
        ...
        <SfListItem v-for="subCat in getSubCategory(category)" class="border-b-2 mb-3 " :key="subCat.id">
...
export default {
  ...
  setup() {
    ...
    const getSubCategory = (category)=> {
      return isMobileMenuOpen.value ? category.items.slice(0, 6) : category.items;
    };
    const closeSideNav = ()=>{
