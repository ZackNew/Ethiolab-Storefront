...
import {
  Context,
  Order,
  UpdateAddressDetailsParams,
  UpdateAddressDetailsResponse,
} from '../../types';
import {
  ARRANGING_PAYMENT_STATE,
  BILLING_TYPE,
  NO_CACHE_FETCH_POLICY,
...
const updateAddressDetails = async (
  context: Context,
  params: UpdateAddressDetailsParams,
  customQuery?: CustomQuery
): Promise<UpdateAddressDetailsResponse> => {
  const { type, input } = params;
  const updateAddressDetailsVariables = { input };
  const isBilling = type === BILLING_TYPE;

  const updateAddressDetailsQuery = isBilling
    ? setOrderBillingAddressMutation
    : setOrderShippingAddressMutation;

  const { updateAddressDetails } = context.extendQuery(customQuery, {
    updateAddressDetails: {
      query: updateAddressDetailsQuery,
      variables: updateAddressDetailsVariables,
    },
  });

  const request = (await context.client.mutate({
    mutation: gql`
      ${updateAddressDetails.query}
    `,
